    ...
    static async validateApiKey(key) {
        ...
        try {
            ...
            if (response.ok || response.status === 400) {
                // 400 is a valid response for a bad request, not a bad key
                return { success: true };
            } else {
 ...
 */
function createStreamingLLM({ apiKey, model = 'claude-3-5-sonnet-20241022', temperature = 0.7, maxTokens = 4096, ...config }) {
    const client = new Anthropic({ apiKey });
            ...
            const stream = new ReadableStream({
                async start(controller) {
                    try {
                        ...
                        console.log(`[Anthropic Provider] Streamed ${chunkCount} chunks, total length: ${totalContent.length} chars`);
